...
def write(card, response):
    ...
    ranked_image.text((x-word.calc_length(tier[0], rank_size)/2, y-20-rank_size), tier[0], font=rank_font, fill=rank.get_colour(response["eloRate"])[0], stroke_fill=rank.get_colour(response["eloRate"])[1], stroke_width=4)
    ranked_image.text((x-word.calc_length(tier[1], division_size)/2, y-20), tier[1], font=division_font, fill=rank.get_colour(response["eloRate"])[0], stroke_fill=rank.get_colour(response["eloRate"])[1], stroke_width=4)
