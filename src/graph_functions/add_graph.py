...
def write(uuid, matches, data_type, season):
    ...
    if data_type == "Elo":
        ...
        data["Completion time (smoothed)"] = pd.to_datetime(
            data["Completion time (smoothed)"], unit="ms"
        )
    ...
    if data_type == "Elo":
        ...
        ax.fill_between(
            x=games_ago, y1=metric, color="#00FFFF", interpolate=True, alpha=0.2
        )
...
def get_comps(uuid, matches, season):
    ...
    for match in matches:
        if (
            match["decayed"]
            or match["result"]["uuid"] != uuid
            or match["forfeited"] != False
        ):
            continue
