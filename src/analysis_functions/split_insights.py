...
def get_death_comments(average_deaths, elo):
    ...
    death_comment = {
        ...
        "value": [f"`{' ' if average_deaths[split] < 0.1 else ''}{numb.round_sf(average_deaths[split] * 100, 3)}%` - {split_mapping[split]}" for split in average_deaths],
        "inline": True,
    }
    rank_comment = {
        "name": f"{ranks[rank_no]} Death Rates",
        "value": [f"`{' ' if overall_deaths[split] < 0.1 else ''}{numb.round_sf(overall_deaths[split] * 100, 3)}%` - {split_mapping[split]}" for split in overall_deaths],
        "inline": True,
    }
